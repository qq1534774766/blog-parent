...
//            jsonParam = DigestUtils.md5Hex(jsonParam);专业的事交给专业的人做，让被调用方执行加密，我们要给负责执行删除缓存方法来执行这个方法，这里不能执行
            log.info("第一次删除缓存 ↓");
            deleteProcess(cacheId, simpleClassName, redisMethodName, jsonParam);
            //原方法执行
        ...
        public void deleteProcess(String[] cacheId, String[] simpleClassName, String[] redisMethodName, String[] jsonParam) {
            if (cacheId.length != simpleClassName.length||cacheId.length !=redisMethodName.length)
                throw new IllegalArgumentException("延迟双删的参数长度不一致");
            for (int i = 0; i < cacheId.length; i++) {
                // 这里使用的是线程池异步方式，但是似乎这样不对，因为延迟双删要的是即时性。纯粹练手
                threadService.deleteRedis(cacheId[i], simpleClassName[i],
                        redisMethodName[i], (jsonParam.length==cacheId.length&&StringUtils.isNotEmpty(jsonParam[i])) ? jsonParam[i] : null);
            }
